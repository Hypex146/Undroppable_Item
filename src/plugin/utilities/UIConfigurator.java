...
public class UIConfigurator {
	...
	public UIConfigurator(UndroppableItem main_plugin) {
	...
	public FileConfiguration getCustomConfig(String pathToFile) {
		...
		if (!folderFile.exists()) {
			folderFile.mkdirs();
			main_plugin_.getUILogger().log(LogLevel.DEBUG, Level.INFO, "Создана папка: " + folderFile.getAbsolutePath());
		}
		if (!configFile.exists()) {
			try {
				configFile.createNewFile();
				main_plugin_.getUILogger().log(LogLevel.DEBUG, Level.INFO, "Создан файл: " + configFile.getAbsolutePath());
			} catch (IOException e) {
				main_plugin_.getUILogger().log(LogLevel.MINIMAL, Level.SEVERE, "Не удалось создать файл (IOException): "
		...
		try {
			...
			main_plugin_.getUILogger().log(LogLevel.DEBUG, Level.INFO, "Считан файл: "+configFile.getAbsolutePath());
		} catch (FileNotFoundException e) {
			e.printStackTrace();
			main_plugin_.getUILogger().log(LogLevel.MINIMAL, Level.SEVERE, "Не удалось считать файл (FileNotFoundException): "
					+configFile.getAbsolutePath());
			e.printStackTrace();
			return null;
		} catch (IOException e) {
			e.printStackTrace();
			main_plugin_.getUILogger().log(LogLevel.MINIMAL, Level.SEVERE, "Не удалось считать файл (IOException): "
					+configFile.getAbsolutePath());
			e.printStackTrace();
			return null;
		} catch (InvalidConfigurationException e) {
			e.printStackTrace();
			main_plugin_.getUILogger().log(LogLevel.MINIMAL, Level.SEVERE, "Не удалось считать файл (InvalidConfigurationException): "
					+configFile.getAbsolutePath());
			e.printStackTrace();
			return null;
		}
	...
	public int saveCustomConfig(String pathToFile, FileConfiguration configToSave) {
		...
		try {
			...
			main_plugin_.getUILogger().log(LogLevel.DEBUG, Level.INFO, "Сохранён файл: "+configFile.getAbsolutePath());
		} catch (IOException e) {
			e.printStackTrace();
			main_plugin_.getUILogger().log(LogLevel.MINIMAL, Level.SEVERE, 
	...
	public boolean getBoolean(FileConfiguration config, String field, boolean defaultValue) {
		if (!config.isBoolean(field)) {
			main_plugin_.getUILogger().log(LogLevel.DEBUG, Level.WARNING, 
